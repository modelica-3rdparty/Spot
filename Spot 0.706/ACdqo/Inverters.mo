...
model Rectifier "Rectifier, 3-phase dqo"
  ...
  replaceable Components.RectifierEquation rectifier "rectifier model"
    ...
    choice(redeclare Spot.ACdqo.Inverters.Components.RectifierEquation
            rectifier "equation, with losses"),
    choice(redeclare Spot.ACdqo.Inverters.Components.RectifierModular rectifier
            "modular, with losses")));
...
equation
  connect(AC, rectifier.AC) annotation (points=[100,0; 10,0],   style(color=62,
          rgbcolor={0,120,120}));
...
model Inverter "Complete modulator and inverter, 3-phase dqo"
  ...
  replaceable Components.InverterSwitch inverter "inverter model"
    annotation (extent=[-10,-10; 10,10], choices(
    choice(redeclare Spot.ACdqo.Inverters.Components.InverterSwitch inverter
            "switch, no diode, no losses"),
    choice(redeclare Spot.ACdqo.Inverters.Components.InverterEquation inverter
            "equation, with losses"),
    choice(redeclare Spot.ACdqo.Inverters.Components.InverterModular inverter
            "modular, with losses")));
  protected
  outer System system;
...
<li>Modular: composed from semiconductor-switches and diodes. These components with ideal V-I characteristic can be replaced by custom-specified semiconductor models.</li>
  ...
  model RectifierEquation "Rectifier equation, 3-phase dqo"
    ...
    Real[3,3] Park = Spot.Base.Transforms.park(AC.theta[2]);
...
Blocking losses are neglected in the expression of dissipated heat <tt>Q_flow</tt>.</p>
  ...
  model RectifierModular "Rectifier modular, 3-phase"
    ...
    replaceable parameter SCpackage.SCparameter par(final Hsw_nom=0)
        "SC parameters"
    annotation (extent=[-80,-80;-60,-60]);
    Nodes.ACdqo_a_b_c acdqo_a_b_c annotation (extent=[80,-10; 60,10]);
...
"), Icon( Text(
  ...
  equation
    connect(AC, acdqo_a_b_c.term) annotation (points=[100,0; 80,0],   style(color=
             62, rgbcolor={0,120,120}));
    connect(acdqo_a_b_c.term_a, diodeMod_a.AC) annotation (points=[60,4; 40,4;
    ...
    connect(acdqo_a_b_c.term_c, diodeMod_c.AC) annotation (points=[60,-4; 40,-4;
            40,-40; 10,-40],   style(color=3, rgbcolor={0,0,255}));
    connect(diodeMod_a.DC, DC) annotation (points=[-10,40; -40,40; -40,0; -100,
            0],
          style(color=3, rgbcolor={0,0,255}));
    connect(diodeMod_b.DC, DC)
        annotation (points=[-10,0; -100,0],
                                          style(color=3, rgbcolor={0,0,255}));
    connect(diodeMod_c.DC, DC) annotation (points=[-10,-40; -40,-40; -40,0;
            -100,0],
          style(color=3, rgbcolor={0,0,255}));
    connect(diodeMod_a.heat, heat_adapt.port_a)   annotation (points=[0,50; 0,
            60; -4,60; -4,74], style(color=42, rgbcolor={176,0,0}));
    connect(diodeMod_b.heat, heat_adapt.port_b)   annotation (points=[0,10; 0,
            20; 20,20; 20,64; 0,64; 0,74], style(color=42, rgbcolor={176,0,0}));
    connect(diodeMod_c.heat, heat_adapt.port_c)   annotation (points=[0,-30; 0,
            -20; 30,-20; 30,74; 4,74], style(color=42, rgbcolor={176,0,0}));
...
"), Icon( Text(
  ...
  equation
    connect(AC, acdqo_a_b_c.term)   annotation (points=[100,0; 80,0],   style(
            color=62, rgbcolor={0,120,120}));
    connect(acdqo_a_b_c.term_a, switchMod_a.AC)   annotation (points=[60,4; 40,
            4; 40,40; 10,40],       style(color=3, rgbcolor={0,0,255}));
    connect(switchMod_a.DC, DC)   annotation (points=[-10,40; -60,40; -60,0;
            -100,0],
          style(color=3, rgbcolor={0,0,255}));
    ...
    connect(switchMod_b.DC, DC)
        annotation (points=[-10,0; -100,0],
                                          style(color=3, rgbcolor={0,0,255}));
    connect(acdqo_a_b_c.term_c, switchMod_c.AC)   annotation (points=[60,-4; 40,
            -4; 40,-40; 10,-40],       style(color=3, rgbcolor={0,0,255}));
    connect(switchMod_c.DC, DC)   annotation (points=[-10,-40; -60,-40; -60,0;
            -100,0],
                style(color=3, rgbcolor={0,0,255}));
    connect(gates, gate3demux1.gates)   annotation (points=[-60,100; -60,80;
            -40,80], style(color=5, rgbcolor={255,0,255}));
    connect(gate3demux1.gates_a, switchMod_a.gates)   annotation (points=[-46,60;
            -46,54; -6,54; -6,50],     style(color=5, rgbcolor={255,0,255}));
    connect(gate3demux1.gates_b, switchMod_b.gates)   annotation (points=[-40,60;
            -40,20; -6,20; -6,10],     style(color=5, rgbcolor={255,0,255}));
    connect(gate3demux1.gates_c, switchMod_c.gates)   annotation (points=[-34,60;
            -34,-20; -6,-20; -6,-30],     style(color=5, rgbcolor={255,0,255}));
    connect(switchMod_a.heat, heat_adapt.port_a)   annotation (points=[0,50; 0,
            60; -4,60; -4,74], style(color=42, rgbcolor={176,0,0}));
    connect(switchMod_b.heat, heat_adapt.port_b)   annotation (points=[0,10; 0,
            20; 20,20; 20,64; 0,64; 0,74], style(color=42, rgbcolor={176,0,0}));
    connect(switchMod_c.heat, heat_adapt.port_c)   annotation (points=[0,-30; 0,
            -20; 30,-20; 30,74; 4,74], style(color=42, rgbcolor={176,0,0}));
    connect(heat_adapt.port_abc, heat)
        annotation (points=[0,86; 0,100], style(color=42, rgbcolor={176,0,0}));
  end InverterModular;
...
partial model AC_DC_base "AC-DC base, 3-phase dqo"
  extends Base.Icons.Inverter_dqo;

  Base.Interfaces.ACdqo_n AC "AC 3-phase connection"
...
partial model SwitchEquation "Switch equation, 3-phase dqo"
  ...
  function loss = Spot.Base.Math.taylor "temp dependence of losses";
  annotation (
    Coordsys(
      extent=
     [-100, -100; 100, 100],
