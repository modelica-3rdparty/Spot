...
<p>In the abc-representation the following relations hold between terminal-voltage v_term and -current i_term on the one hand and conductor-voltage v_cond and -current i_cond on the other:</p>
<p>A) Y-topology.</p>
<pre>
...
<p>where <tt>perm</tt> denotes a permutation of phases corresponding to <tt>(-1,0,+1)*120deg</tt>  phase shift.<br>
The default value is <tt>sh = 0</tt> and corresponds to a rotation of the dq-signals by 30deg in positive sense.</p>
...
"),
  ...
  model TrafoIdeal "Ideal transformer, 3-phase abc"
    parameter Boolean D_D=false "set true if Delta-Delta topology!";
    extends Partials.TrafoIdealBase(final stIni_en=false, i1(start={1,1,1}));
    annotation (
...
"),
  ...
  model TrafoStray "Ideal magnetic coupling transformer, 3-phase abc"
...
<pre>  R_nom[k] = V_nom[k]^2/S_nom,  k = 1(primary), 2(secondary)</pre>
  ...
  initial equation
    if steadyIni_t then
      der(i1) = omega[1]*j_abc(i1);
    end if;

  equation
    i1 + i2 = zeros(3);
    if system.transientSim then
      (L1 + L2)*der(i1) + omega[2]*(L1 + L2)*j_abc(i1) + sum(R)*i1 = v1 - v2;
    else
      omega[2]*(L1 + L2)*j_abc(i1) + sum(R)*i1 = v1 - v2;
    end if;
  end TrafoStray;

  model TrafoMag "Magnetic coupling transformer, 3-phase abc"
...
<pre>  R_nom[k] = V_nom[k]^2/S_nom,  k = 1(primary), 2(secondary)</pre>
  ...
  initial equation
    if steadyIni_t then
      der(i1) = omega[1]*j_abc(i1);
      der(i2) = omega[1]*j_abc(i2);
      der(imag) = omega[1]*j_abc(imag);
  ...
  equation
    ...
    Redc*iedc = v0;
    if system.transientSim then
      L1*der(i1) + omega[2]*L1*j_abc(i1) + R[1]*i1 = v1 - v0;
      L2*der(i2) + omega[2]*L2*j_abc(i2) + R[2]*i2 = v2 - v0;
      Lm*der(imag) + omega[2]*Lm*j_abc(imag) = v0;
    else
      omega[2]*L1*j_abc(i1) + R[1]*i1 = v1 - v0;
      omega[2]*L2*j_abc(i2) + R[2]*i2 = v2 - v0;
      omega[2]*Lm*j_abc(imag) = v0;
    end if;
  end TrafoMag;
...
"),   Icon,
  ...
  equation
    ...
    if system.transientSim then
      L1*der(i1) + omega[2]*L1*j_abc(i1) + R[1]*i1 = v1 - v0;
      L2*der(i2) + omega[2]*L2*j_abc(i2) + R[2]*i2 = v2 - v0;
      gp*(der(psi0) + omega[2]*j_abc(psi0)) = v0;
    else
      omega[2]*L1*j_abc(i1) + R[1]*i1 = v1 - v0;
      omega[2]*L2*j_abc(i2) + R[2]*i2 = v2 - v0;
      gp*(omega[2]*j_abc(psi0)) = v0;
    end if;
  end TrafoSatEff;

  model TrafoSat "Saturation transformer, 3-phase abc"
    extends Partials.TrafoSatBase;
...
"),
  ...
  equation
    ...
    Redc*iedc = v0;
    psi0 = Lm*imag;
    gp = der_sat(Rot*psi0/psi_nom, c_sat);

    L1*der(i1) + omega[2]*L1*j_abc(i1) + R[1]*i1 = v1 - v0;
    L2*der(i2) + omega[2]*L2*j_abc(i2) + R[2]*i2 = v2 - v0;
    transpose(Rot)*diagonal(gp)*Rot*(der(psi0) + omega[2]*j_abc(psi0)) = v0;
  end TrafoSat;
...
initial equation
  if steadyIni_t then
    der(i2a) = omega[1]*j_abc(i2a);
    der(i2b) = omega[1]*j_abc(i2b);
  end if;
...
equation
  ...
  if system.transientSim then
    L1*der(i1) + omega[2]*L1*j_abc(i1) + R[1]*i1 = v1 - v0;
    L2a*der(i2a) + omega[2]*L2a*j_abc(i2a) + R[2]*i2a = v2a - v0;
    L2b*der(i2b) + omega[2]*L2b*j_abc(i2b) + R[3]*i2b = v2b - v0;
  else
    omega[2]*L1*j_abc(i1) + R[1]*i1 = v1 - v0;
    omega[2]*L2a*j_abc(i2a) + R[2]*i2a = v2a - v0;
    omega[2]*L2b*j_abc(i2b) + R[3]*i2b = v2b - v0;
  end if;
end Trafo3Stray;
  ...
  package Partials "Partial models"
    ...
    partial model TrafoIdealBase "Base for ideal transformer, 3-phase abc"
      extends Ports.YDportTrafo_p_n(
        w1(start=w1_set), w2(start=w2_set),
        final term_p(v(start={cos(system.alpha0),cos(system.alpha0-2*pi/3),cos(system.alpha0-4*pi/3)}*par.V_nom[1])),
        final term_n(v(start={cos(system.alpha0),cos(system.alpha0-2*pi/3),cos(system.alpha0-4*pi/3)}*par.V_nom[2])));
...
<p>Set <tt>dynTC = true</tt> if tap-index changes during simulation.</p>
    ...
    partial model TrafoStrayBase
      "Base for ideal magnetic coupling transformer, 3-phase abc"
      extends TrafoIdealBase(redeclare replaceable parameter
          Spot.ACabc.Transformers.Parameters.TrafoStray par)
        annotation (extent=[-80,60; -60,80]);
    protected
      final parameter SIpu.Inductance[2] x_s=(2*par.x + par.x0)/3;
      final parameter SIpu.Inductance[2] x_m=-(par.x - par.x0)/3;
      final parameter SI.Resistance[2] R=par.r.*RL_base[:, 1];
      final parameter SI.Inductance[3,3] L1=[x_s[1],x_m[1],x_m[1];x_m[1],x_s[1],x_m[1];x_m[1],x_m[1],x_s[1]]*RL_base[1,2];
      final parameter SI.Inductance[3,3] L2=[x_s[2],x_m[2],x_m[2];x_m[2],x_s[2],x_m[2];x_m[2],x_m[2],x_s[2]]*RL_base[2,2];
      annotation (
        Coordsys(
    extent=[-100,-100; 100,100],
    grid=[2,2],
...
<p>Precalculation of coefficients for ideal magnetic coupling transformer</p>
    ...
    partial model TrafoMagBase
      "Base for magnetic coupling transformer, 3-phase abc"
      extends TrafoStrayBase(redeclare replaceable parameter
          Spot.ACabc.Transformers.Parameters.TrafoMag par)
        annotation (extent=[-80,60; -60,80]);
    protected
...
<p>Precalculation of coefficients for magnetic coupling trafo transformer</p>
    ...
    partial model TrafoSatBase "Base for saturation transformer, 3-phase abc"
      extends TrafoMagBase(redeclare replaceable parameter
          Spot.ACabc.Transformers.Parameters.TrafoSat par)
        annotation (extent=[-80,60; -60,80]);
    protected
...
<p>Precalculation of coefficients for saturation transformer</p>
  ...
  partial model Trafo3IdealBase
    ...
    extends Ports.YDportTrafo_p_n_n(
      ...
      final term_p(v(start={cos(system.alpha0),cos(system.alpha0-2*pi/3),cos(system.alpha0-4*pi/3)}*par.V_nom[1])),
      final term_na(v(start={cos(system.alpha0),cos(system.alpha0-2*pi/3),cos(system.alpha0-4*pi/3)}*par.V_nom[2])),
      final term_nb(v(start={cos(system.alpha0),cos(system.alpha0-2*pi/3),cos(system.alpha0-4*pi/3)}*par.V_nom[3])));
...
<p>Set <tt>dynTC = true</tt> if tap-index changes during simulation.</p>
  ...
  partial model Trafo3StrayBase
    ...
    extends Trafo3IdealBase(redeclare replaceable parameter
          Spot.ACabc.Transformers.Parameters.Trafo3Stray par)
      annotation (extent=[-80,60; -60,80]);
    protected
    final parameter SIpu.Inductance[3] x_s=(2*par.x + par.x0)/3;
    final parameter SIpu.Inductance[3] x_m=-(par.x - par.x0)/3;
    final parameter SI.Resistance[3] R=par.r.*RL_base[:, 1];
    final parameter SI.Inductance[3,3] L1=[x_s[1],x_m[1],x_m[1];x_m[1],x_s[1],x_m[1];x_m[1],x_m[1],x_s[1]]*RL_base[1,2];
    final parameter SI.Inductance[3,3] L2a=[x_s[2],x_m[2],x_m[2];x_m[2],x_s[2],x_m[2];x_m[2],x_m[2],x_s[2]]*RL_base[2,2];
    final parameter SI.Inductance[3,3] L2b=[x_s[3],x_m[3],x_m[3];x_m[3],x_s[3],x_m[3];x_m[3],x_m[3],x_s[3]]*RL_base[3,2];
    annotation (
      Coordsys(
  extent=[-100,-100; 100,100],
  grid=[2,2],
