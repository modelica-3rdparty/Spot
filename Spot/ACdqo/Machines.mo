...
"), Icon);
  ...
  model Asynchron "Asynchronous machine, cage-rotor, 3-phase dqo"
    extends Partials.AsynchronBase(w_el(start=300));
...
<p>More information see Partials.AsynchronBase.</p>
  ...
  model AsynchronY_D "Asynchronous machine Y-Delta, cage-rotor, 3-phase dqo"
    extends Partials.AsynchronBase(redeclare Spot.ACdqo.Ports.Topology.Y_Delta
        top "Y-Delta");
...
"),
  ...
  model Synchron3rd_el "Synchronous machine el, 3rd order model, 3-phase dqo"
    extends Partials.Synchron3rdBase(final phi_el_ini=-pi/2+system.alpha0,
      redeclare replaceable parameter
        Spot.ACdqo.Machines.Parameters.Synchron3rd_el par);
...
<p>More information see Partials.Synchron3rdBase.</p>
  ...
  model Synchron_el "Synchronous machine el, 3-phase dqo"
    extends Partials.SynchronBase(final phi_el_ini=-pi/2+system.alpha0,
      redeclare replaceable parameter
        Spot.ACdqo.Machines.Parameters.Synchron_el par);
...
<p>More information see Partials.SynchronBase.</p>
  ...
  Rectangle(extent=[-50,-42; -30,-46],
        ...
        Line(points=[-50,-44; -70,-44], style(color=3, rgbcolor={0,0,255})),
        Line(points=[30,-44; 40,-44; 40,-36; -70,-36], style(color=3,
              rgbcolor={0,0,255})),
        Text(
          extent=[30,-39; 90,-49],
  ...
  model Synchron3rd_pm "Synchronous machine pm, 3rd order model, 3-phase dqo"
    extends Partials.Synchron3rdBase(redeclare replaceable parameter
        Spot.ACdqo.Machines.Parameters.Synchron3rd_pm par);
...
<p>More information see Partials.Synchron3rdBase.</p>
  ...
  model Synchron_pm "Synchronous machine pm, 3-phase dqo"
    extends Partials.SynchronBase(redeclare replaceable parameter
        Spot.ACdqo.Machines.Parameters.Synchron_pm par);
  ...
  protected
    ...
    annotation (Icon(
           Rectangle(extent=[-90,112; 90,88], style(
            ...
            rgbcolor={0,0,127},
            fillColor=68,
            rgbfillColor={170,213,255}))), Documentation(info="<html>
...
"),     Icon(
    ...
    protected
      ...
      final parameter Coefficients.Asynchron c=Base.Precalculation.machineAsyn(par,n_r,top.scale);
...
<pre>
  ...
  omega[2] = der(term.theta[2])
    ...
    equation
      psi_s = diagonal(c.L_s[1:2])*i[1:2] + {c.L_m*i_rd, c.L_m*i_rq};
      psi_rd = c.L_m*i[1] + c.L_r*i_rd;
      psi_rq = c.L_m*i[2] + c.L_r*i_rq;
      ...
      heat.port.Q_flow = -{c.R_s*i*i, diagonal(c.R_r)*i_rd*i_rd + diagonal(c.R_r)*i_rq*i_rq};
    end AsynchronBase;
...
<pre>
  v_s, i_s:    stator-voltage and -current dqo in the rotor frame of the machine.
    ...
    equation
      Rot_dq = Base.Transforms.rotation_dq(phi_el - term.theta[2]);
    ...
    partial model Synchron3rdBase "Synchronous machine 3rd base, 3-phase dqo"
      ...
      replaceable parameter Parameters.Synchron3rd par(f_nom=system.f_nom)
        ...
        annotation (extent=[-60,60; -40,80]);
    protected
      final parameter Coefficients.Synchron3rd c = Base.Precalculation.machineSyn3rd(par,top.scale);
      SI.MagneticFlux[2] psi_s "magnetic flux stator dq";
      annotation (
        Coordsys(
          extent=
...
<pre>
  powerAngle:   difference (angle of rotor) - (angle of terminal_voltage)
    ...
    protected
      ...
      final parameter Coefficients.Synchron c=Base.Precalculation.machineSyn(par,n_d,n_q,top.scale);
...
&nbsp; &nbsp; For pu-input refer to stator base value <tt>R_base</tt>.</p>
    ...
    Diagram(
        ...
        Line(points=[90,50; 90,40], style(
            ...
            rgbfillColor={0,0,255},
            fillPattern=1)),
        Line(points=[-40,10; 90,10; 90,20],  style(
            color=3,
            rgbcolor={0,0,255},
            fillColor=3,
            rgbfillColor={0,0,255},
        ...
        Line(points=[10,-20; 10,-90], style(
            ...
            rgbfillColor={0,0,255},
            fillPattern=1)),
        Line(points=[-40,-90; 90,-90; 90,-80],  style(
            color=3,
            rgbcolor={0,0,255},
            fillColor=3,
            rgbfillColor={0,0,255},
        ...
        Text(
          ...
          style(
            ...
            fillPattern=1),
          string="r_r3"),
        Line(points=[-40,80; 90,80; 90,70], style(
            color=3,
            rgbcolor={0,0,255},
            fillColor=3,
            rgbfillColor={0,0,255},
  ...
  Rf = resistance field winding   (in Ohm, true value, not scaled)
    ...
    Diagram(
        ...
        Line(points=[-40,-90; 90,-90; 90,-80],  style(
            ...
            rgbfillColor={0,0,255},
            fillPattern=1)),
        Line(points=[-40,-20; 90,-20; 90,-30], style(
            color=3,
            rgbcolor={0,0,255},
            fillColor=3,
            rgbfillColor={0,0,255},
...
record Synchron_pm "Synchronous machine pm parameters"
  ...
  parameter SI.Time[:] to_q={0.0278521}
        "time constant open-loop q-axis {to_q', to_q'', ..}";
  final parameter Boolean use_if0=false
        "induced field current and phase available?";
  final parameter SIpu.Current if0=0 "induced field current at v_s=Vnom/0deg";
  final parameter SIpu.Angle_deg if0_deg=0
        "angle(if0) at v_s=Vnom/0deg (sign: i_f behind v_s)";
  parameter Real tol=1e-6 "tolerance precalculation";
