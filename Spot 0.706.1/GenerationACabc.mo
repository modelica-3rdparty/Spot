  ...
  package Partials "Partial models"
    ...
    initial equation
      if iniType == Spot.Base.Types.v_alpha then
        sqrt(term.v*term.v) = v_ini*generator.par.V_nom;
        atan2(Base.Transforms.Park0[2,:]*term.v, Base.Transforms.Park0[1,:]*term.v) = alpha_ini;
      elseif iniType == Spot.Base.Types.v_p then
        sqrt(term.v*term.v) = v_ini*generator.par.V_nom;
        -term.v*term.i = p_ini*generator.par.S_nom;
      elseif iniType == Spot.Base.Types.v_q then
        sqrt(term.v*term.v) = v_ini*generator.par.V_nom;
        j_abc(term.v)*term.i = q_ini*generator.par.S_nom;
