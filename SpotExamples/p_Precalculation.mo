...
model Z_matrixTrDat0 "z-matrix from transient data, n=1, 0"
  ...
  final parameter SI.Angle alpha_if0=(if0_deg + 180)*d2r
      "mathematical sign convention";
  function T_open = Spot.Base.Precalculation.T_open;
  function z_matrix = Spot.Base.Precalculation.z_fromTransDat;

algorithm
  (zr_d, zx_d) := z_matrix(n_d, omega_nom*tc_d, T_open(x_d, xtr_d, omega_nom*tc_d), x_d, xsig_s, r_s, if0, alpha_if0, 0, true);
...
<pre>
  ...
  r_r    for r:      rotor resistances
        ...
        Line(points=[90,50; 90,40], style(
            ...
            rgbfillColor={0,0,255},
            fillPattern=1)),
        Line(points=[-100,10; 90,10; 90,20], style(
            color=3,
            rgbcolor={0,0,255},
            fillColor=3,
            rgbfillColor={0,0,255},
        ...
        Text(
          ...
          style(
            ...
            fillPattern=1),
          string="v_f"),
        Line(points=[-80,80; 90,80; 90,70], style(
            color=3,
            rgbcolor={0,0,255},
            fillColor=3,
            rgbfillColor={0,0,255},
...
<pre>
  ...
  r_r    for r:      rotor resistances
        ...
        Line(points=[-40,-20; -40,-90], style(
            ...
            rgbfillColor={0,0,255},
            fillPattern=1)),
        Line(points=[-100,-90; 90,-90; 90,-80], style(
            color=3,
            rgbcolor={0,0,255},
            fillColor=3,
            rgbfillColor={0,0,255},
            fillPattern=1)),
        Line(points=[-80,-20; 90,-20; 90,-30], style(
            color=3,
            rgbcolor={0,0,255},
            fillColor=3,
            rgbfillColor={0,0,255},
...
model Z_matrixTrDat2 "z-matrix from transient data, n=2"
  ...
  annotation(Dialog(enable=use_trans and n_d>1));
  parameter SIpu.Angle_deg if0_deg=-101.489837079382
      "angle(if0) at v_s=Vnom/0deg (sign: i_f behind v_s)"
                                                         annotation(Dialog(enable=use_trans and n_d>1));
...
algorithm
  (zr_d, zx_d) := z_matrix(n_d, omega_nom*tc_d, T_open(x_d, xtr_d, omega_nom*tc_d), x_d, xsig_s, r_s, if0, alpha_if0, tol, true);
  (zr_q, zx_q) := z_matrix(n_q, omega_nom*tc_q, T_open(x_q, xtr_q, omega_nom*tc_q), x_q, xsig_s, r_s, 0, 0, 0, false);
  annotation (Documentation(info="<html>
<p>Equivalent circuit on <b>diagram layer</b>!</p>
...
model Z_matrixTrDat3 "z-matrix from transient data, n=3"
  ...
  parameter SIpu.Current if0=0.47426409312074
      "induced field current at v_s=V_nom/0deg"
                                               annotation(Dialog(enable=use_trans and n_d>1));
  parameter SIpu.Angle_deg if0_deg=-148.179736268733
      "angle(if0) at v_s=Vnom/0deg (sign: i_f behind v_s)"
                                                         annotation(Dialog(enable=use_trans and n_d>1));
  parameter Real tol=1e-6 "tolerance precalculation"
    annotation(Dialog(enable=use_trans));
...
model Z_matrixEqCirc0 "z-matrix from equivalent circuit, n=1, 0"
  ...
  function z_matrix = Spot.Base.Precalculation.z_fromEqCirc;
...
model TransDatFromEqCirc "Calculates transient data from equivalent circuit"
  ...
  output SIpu.Reactance[n_d] xtr_d(unit="pu") "transient reactance d-axis";
  output SI.Time[n_q] tc_q "time constant closed-loop q-axis";
  output SI.Time[n_q] to_q "time constant open-loop q-axis";
  output SIpu.Reactance[n_q] xtr_q(unit="pu") "transient reactance q-axis";
  protected
  final parameter Boolean use_trans=false;
  final parameter Integer n_d=size(xsig_rd,1);
  final parameter Integer n_q=size(xsig_rq,1);
  function transientData=Spot.Base.Precalculation.transientData;
...
model EqCircFromTransDat "Calculates equivalent circuit from transient data"
  ...
  output SIpu.Resistance[n_d] r_rd(unit="pu");
  output SIpu.Reactance[n_d] xsig_rd(unit="pu");
  output SIpu.Reactance[n_d+1] xm_d(unit="pu");
  output SIpu.Resistance[n_q] r_rq(unit="pu");
  output SIpu.Reactance[n_q] xsig_rq(unit="pu");
  output SIpu.Reactance[n_q+1] xm_q(unit="pu");
  ...
  final parameter SI.Angle alpha_if0=(if0_deg + 180)*d2r
      "mathematical sign convention";
  function T_open = Spot.Base.Precalculation.T_open;
  function equiCircuit = Spot.Base.Precalculation.equiCircuit;

algorithm
  (r_rd, xsig_rd, xm_d) := equiCircuit(n_d, omega_nom*tc_d, T_open(x_d, xtr_d, omega_nom*tc_d), x_d, xsig_s, r_s, if0, alpha_if0, tol, true);
